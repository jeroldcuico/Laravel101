<form id="delete-disable-{{$id}}" method="POST" action="{{ $action ?? '#' }}">
    @csrf
...
@if(isset($isDeleted) && $isDeleted)
<x-danger-button form="delete-disable-{{$id}}">
...
<x-modal name="yesno_delete{{$id}}" class="justify-center" focusable maxWidth="md">
    <div class="p-5 text-center">
        ...
        <x-primary-button class="mx-3" x-on:click="$dispatch('close')" form="delete-disable-{{$id}}">{{ __('Yes') }}</x-primary-button>
    </div>
</x-modal>